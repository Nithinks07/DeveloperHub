...
import { pullRequestsApi, type PRComment } from "@/services/pull_requests";
...
function CommentNode({
  ...
  const replyMutation = useMutation({
    mutationFn: () => pullRequestsApi.addComment(prId, { content: replyContent, parent_comment_id: comment.id }),
    onSuccess: () => {
  ...
  return (
    <div className={`pt-4 ${level > 0 ? "ml-6 pl-4 border-l border-white/10 mt-2" : "mt-6 border-t border-white/10"}`}>
      <div className="flex items-start gap-3">
        <div className="size-8 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-xs font-bold text-white shrink-0">
          ...
          {comment.replies?.length > 0 && (
            <div className="mt-2">
              {comment.replies.map(reply => (
                <CommentNode key={reply.id} comment={reply} prId={prId} level={level + 1} />
              ))}
            </div>
          )}
        </div>
...
export default function PRCommentThread({
  comments,
  prId,
}: {
  comments: PRComment[];
  ...
  const mutation = useMutation({
    mutationFn: () => pullRequestsApi.addComment(prId, { content: newComment, parent_comment_id: null }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["pull_request", prId] });
      setNewComment("");
    },
  ...
  return (
    <div className="space-y-6">
      <div>
        {comments.map(c => (
          <CommentNode key={c.id} comment={c} prId={prId} />
        ))}
      </div>
