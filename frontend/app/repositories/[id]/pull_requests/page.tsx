...
import { pullRequestsApi, type PRStatus } from "@/services/pull_requests";
import PRCreateModal from "@/components/pull_requests/PRCreateModal";

function StatusIcon({ status }: { status: PRStatus }) {
  switch (status) {
    case "open": return <span className="text-emerald-400">○</span>;
    case "merged": return <span className="text-purple-400">⚡</span>;
    case "closed": return <span className="text-red-400">✗</span>;
...
export default function PullRequestsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["pull_requests", repoId, page],
    queryFn: () => pullRequestsApi.list(repoId, page, 20),
    enabled: Boolean(repoId),
  });
    ...
    <main className="min-h-screen bg-[#080a0f] text-white">
      <nav className="border-b border-white/6 bg-[#080a0f]/80 backdrop-blur-xl sticky top-0 z-30">
        <div className="max-w-6xl mx-auto px-6 h-14 flex items-center gap-2 text-sm">
          ...
          <span className="text-white/20">/</span>
          <span className="text-white/70 font-medium">Pull Requests</span>
        </div>
      </nav>
      ...
      <div className="max-w-6xl mx-auto px-6 py-10">
        <div className="flex items-start justify-between mb-8">
          <div>
            ...
            <p className="mt-1 text-white/40">Review and merge code changes.</p>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="bg-[#0f1117] border border-white/10 rounded-2xl overflow-hidden shadow-2xl">
            ...
            <div className="divide-y divide-white/6">
              {data?.items.map(pr => (
                <div key={pr.id} className="p-5 hover:bg-white/[0.02] transition-colors flex items-start gap-4 group">
                  <div className="mt-1"><StatusIcon status={pr.status} /></div>
                  <div className="flex-1 min-w-0">
                    <Link href={`/pull_requests/${pr.id}`} className="text-base font-semibold text-white hover:text-indigo-400 transition-colors">
                      {pr.title}
                    </Link>
                    <div className="mt-1.5 flex items-center gap-3 text-xs text-white/40">
                      <span>#{pr.number} opened on {new Date(pr.created_at).toLocaleDateString()}</span>
                      <span className="px-2 py-0.5 rounded-md border border-white/10 bg-white/5 font-mono text-[10px]">
      ...
      <PRCreateModal isOpen={modalOpen} onClose={() => setModalOpen(false)} repoId={repoId} />
    </main>
  );
}
